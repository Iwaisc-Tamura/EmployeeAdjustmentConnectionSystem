...
using System.Data.SqlClient;                        //2017-08-31 iwai-tamura add -----
using System.IO;
    ...
    public class ApproverInsert {
        ...
        public void DoInsertApprover(int year, string empNo, string actionType, LoginUser lu, string durationParam) {
            try {
                ...
                //2017-08-31 iwai-tamura add str -----
                //決裁権限自動作成処理　上記旧処理はdataのみ作成　以下処理で全て上書きする
                string spname = "";
                if (actionType == "Objective"){
                    spname = "SD_SP目標管理決裁権限作成";
                }else{
                    //2017-10-31 iwai-tamura upd str -----
                    //2019-10-02 iwai-tamura upd str -----
                    //目標と上司の条件が変わった為、常に職能判定を利用する
                    spname = "SD_SP職能判定決裁権限作成";
                    //if (durationParam == "1"){
                    //    //夏期(下期)は前年の年末データを利用する。
                    //    spname = "SD_SP職能判定決裁権限作成";
                    //}else{
                    //    //年末(上期)は直近の目標データを利用する。
                    //    spname = "SD_SP職能判定決裁権限作成_目標";
                    //}
                    //2019-10-02 iwai-tamura upd end -----
                    //spname = "SD_SP職能判定決裁権限作成";
                    //2017-10-31 iwai-tamura upd end -----
                }
                using(DbManager dm = new DbManager())
                using(IDbCommand cmd = dm.CreateCommand(spname)) {
                    ...
                    nlog.Debug(spname + " start");
                    int result = cmd.ExecuteNonQuery();
                    nlog.Debug(spname + " end");
                }
                //2017-08-31 iwai-tamura add end -----
        ...
        private List<string> CreateInsSqlList(int year, string empNo, DataTable usTable, DataTable apTable) {
            try {
                ...
                foreach(DataRow row in usTable.Rows) {
                    ...
                    //目標、職能ともにストアド項目をセット
                    //2017-05-27 iwai-tamura add str -----
                    objPartialSql += "'2014-01-01','2014-01-01',0,"
                                   ...
                                   + "NULL),";
                    //objPartialSql += "'2014-01-01','2014-01-01',0),";
                    //2017-05-27 iwai-tamura add end -----
