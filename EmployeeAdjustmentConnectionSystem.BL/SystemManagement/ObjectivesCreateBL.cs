...
namespace EmployeeAdjustmentConnectionSystem.BL.SystemManagement {
    ...
    /// 目標管理データ作成ビジネスロジック
    /// </summary>
    public class ObjectivesCreateBL {
        ...
        public string CreateObjective(SystemManagementModels model, string button,LoginUser lu) {
            try {
                //開始
                ...
                //ストアド実行
                DoObjectivesStored((int)yearVal, employee,actionType,lu);

                return "目標管理初期データを作成しました。";
            } catch(Exception ex) {
                // エラー
        ...
        private void DoObjectivesStored(int yearParam, string employeeParam,string actionType,LoginUser lu) {
            try {

                //開始
                nlog.Debug(System.Reflection.MethodBase.GetCurrentMethod().Name + " start");        

                string sql = "SD_SP目標管理初期作成";
