...
namespace EmployeeAdjustmentConnectionSystem.BL.SystemManagement {
    public class TakeOverMoveBL {
        ...
        /// <returns></returns>
        public bool ObjectiveTakeOverMoveFileCheck(SystemManagementModels model , ArrayList ReadData) {
            try {
                //開始
                //比較用項目名
                string[] aryFileItemName = new string[] { "前任社員番号", "前任所属番号", "後任社員番号","後任所属番号" };

                nlog.Debug(System.Reflection.MethodBase.GetCurrentMethod().Name + " start");
                foreach (string[] headCheck in ReadData) {
                    if (headCheck.Length == 4){
        ...
        /// <summary>
        /// 目標管理 異動による引継ぎ一括処理　データチェック
        /// </summary>
        /// <param name="model"></param>
        /// <param name="ReadData"></param>
        /// <returns></returns>
        public string ObjectiveTakeOverMoveDataCheck(SystemManagementModels model , ArrayList ReadData) {
            try {
                //開始
                nlog.Debug(System.Reflection.MethodBase.GetCurrentMethod().Name + " start");
                ...
                if (model.TakeOverMoveBulkUploadFile.FileName.EndsWith("xls") || model.TakeOverMoveBulkUploadFile.FileName.EndsWith("xlsx")) {
                    ...
                    foreach (string[] data in ReadData) {
                        //前任者存在チェック
                        switch (ObjectiveConfirm(model.TakeOverMoveBulkYaer,data[0],data[1]))
                        {
                            case "Nothing": //存在しない
                                cntNothing ++;
                                if (cntNothing<6){
                        ...
                        //後任者存在チェック
                        switch (ObjectiveConfirm(model.TakeOverMoveBulkYaer,data[2],data[3]))
                        {
                            case "Nothing": //存在しない
                                cntNothing ++;
                                if (cntNothing<6){
                                    varNothing += " 社員番号:" + data[2] + " 所属番号:" + data[3] + "<br>";
                                }
                                break;
                            case "Committed": //確定済み
                                cntCommitted ++;
                                if (cntCommitted<6){
                                    varCommitted += " 社員番号:" + data[2] + " 所属番号:" + data[3] + "<br>";
                                }
                                break;
        ...
        /// <param name="ReadData"></param>
        /// <returns></returns>
        public bool ObjectiveTakeOverMoveSingle(SystemManagementModels model) {
            try {
                //開始
                nlog.Debug(System.Reflection.MethodBase.GetCurrentMethod().Name + " start");
                string sql = "SD_SP目標管理DATA異動引継処理";
                ...
                using (var scope = new TransactionScope()) {
                    ...
                    using (DataSet ds = new DataSet()) {
                        ...
                        string year = model.TakeOverMoveSingleYaer; // 年度
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add(new SqlParameter("YearParam", SqlDbType.Int));
                        cmd.Parameters.Add(new SqlParameter("befEmployeeParam", SqlDbType.NVarChar));
                        cmd.Parameters.Add(new SqlParameter("befBranchParam", SqlDbType.NVarChar));
                        cmd.Parameters.Add(new SqlParameter("aftEmployeeParam", SqlDbType.NVarChar));
                        cmd.Parameters.Add(new SqlParameter("aftBranchParam", SqlDbType.NVarChar));
                        var parameters = cmd.Parameters.Cast<IDbDataParameter>().ToArray<IDbDataParameter>();
                        parameters[0].Value = int.Parse(year);                  // 年度
                        parameters[1].Value = DataConv.IfNull(model.TakeOverMoveSingleBefEmployeeNo);         // 前任社員番号
                        parameters[2].Value = DataConv.IfNull(model.TakeOverMoveSingleBefDepartment);         // 前任所属番号
                        parameters[3].Value = DataConv.IfNull(model.TakeOverMoveSingleAftEmployeeNo);         // 後任社員番号
                        parameters[4].Value = DataConv.IfNull(model.TakeOverMoveSingleAftDepartment);         // 後任所属番号
                        cmd.ExecuteNonQuery();
                    }
                    scope.Complete();
                    return true;
        ...
        /// <param name="ReadData"></param>
        /// <returns></returns>
        public bool ObjectiveTakeOverMoveBulk(SystemManagementModels model , ArrayList ReadData) {
            try {
                //開始
                nlog.Debug(System.Reflection.MethodBase.GetCurrentMethod().Name + " start");
                string sql = "SD_SP目標管理DATA異動引継処理";
                ...
                using (var scope = new TransactionScope()) {
                    ...
                    using (DataSet ds = new DataSet()) {
                        ...
                        string year = model.TakeOverMoveBulkYaer; // 年度
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add(new SqlParameter("YearParam", SqlDbType.Int));
                        cmd.Parameters.Add(new SqlParameter("befEmployeeParam", SqlDbType.NVarChar));
                        cmd.Parameters.Add(new SqlParameter("befBranchParam", SqlDbType.NVarChar));
                        ...
                        if (model.TakeOverMoveBulkUploadFile.FileName.EndsWith("xls") || model.TakeOverMoveBulkUploadFile.FileName.EndsWith("xlsx")) {
                            ...
                            foreach (string[] data in ReadData) {
                                var parameters = cmd.Parameters.Cast<IDbDataParameter>().ToArray<IDbDataParameter>();
                                parameters[0].Value = int.Parse(year);                  // 年度
                                parameters[1].Value = DataConv.IfNull(data[0]);         // 前任社員番号
                                parameters[2].Value = DataConv.IfNull(data[1]);         // 前任所属番号
                                parameters[3].Value = DataConv.IfNull(data[2]);         // 後任社員番号
                                parameters[4].Value = DataConv.IfNull(data[3]);         // 後任所属番号
                                cmd.ExecuteNonQuery();
                            }
                        }
                    }
