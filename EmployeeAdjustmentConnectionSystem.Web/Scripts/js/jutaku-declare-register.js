...
function checkAll() {
    ...
    ///住宅借入金等特別控除申告書チェック
    //入力項目チェック
    aryCheckInput = ["Head_HousingLoanSpecialDeduction_ApplyCount"
        , "Head_HousingLoanSpecialDeduction_ResidenceStart1DateYear"
        , "Head_HousingLoanSpecialDeduction_ResidenceStart1DateMonth"
        , "Head_HousingLoanSpecialDeduction_ResidenceStart1DateDay"
        , "Head_HousingLoanSpecialDeduction_Type1"
        , "Head_HousingLoanSpecialDeduction_YearEndBalance1"
    ];
    $.each(aryCheckInput, function (index, value) {
        if (document.getElementById(value).value == "") {
            message = '必須項目が入力されていません。<br/>確認してください。';
